...
if (!empty($_POST['db-btn']))
{
   ...
   if ($_POST['db-btn'] == "Search for My Driver (Email)")
   {
   	  if (!empty($_POST['email'])) {
		  $driver = getDriverByEmail($_POST['email']);
		  $driver_found = true; 
	  }
	  else {
		  $msg = "No Driver found.";
   ...
   if ($_POST['db-btn'] == "Search for My Driver (Username)")
   {
   	  if (!empty($_POST['username'])) {
		  $driver = getDriverByUsername($_POST['username']);
		  $driver_found = true; 
	  }
	  else {
		  $msg = "No Driver found.";
...
<div class="col-sm-10">
	...
	<br>
	   ...
	   <form action="DeliveryDriverUser.php" method="post">  
	   ...
	   <div class="form-group">
		<input type="submit" value="Search for My Driver (Username)" class="btn btn-dark" name="db-btn" title="Search from relation" />
		<input type="submit" value="Search for My Driver (Email)" class="btn btn-dark" name="db-btn" title="Search from relation" />
		  <div class="row">
		  <div class="col-sm-4">
		  <br>
		  <br>
		  <table <?php if ($driver_found)?> class="table table-light">
			<tr>
			<th>Employee ID</th>
			<th>Employee Name</th>
			</tr>
			<tr>
				<td>
				  <?php echo $driver['employeeID']; // refer to column name in the table ?> 
				</td>
				<td>
				  <?php 
				  
				  echo getDDUbyEmpID($driver['employeeID'])['first_last']; // refer to column name in the table  
				  ?>
				</td>
			</tr>
		  </table>
